...
# TODO: Allow individual server coloring, Fix !changeclass for each server.
# Add more details to MPA log, such as user ID on who started and closed the MPA. Possibly a date and time stamp.
...
async def generateList(message, inputstring):
 ...
 # This is the Ishana format
    ...
    if message.server.id == '153346891109761024' or message.server.id == '159184581830901761' or message.server.id == '189900848862724096':
        for word in EQTest[message.channel.id]:
            if (type(word) is PlaceHolder):
                if message.server.id == '159184581830901761':
                    color[message.channel.id] = 0x0196ef
                    playerlist += (inactiveSlots[0] + '\n')
                    classlist += (classes[11] + '\n')
                elif message.server.id == '153346891109761024':
                    color[message.channel.id] = 0xffb3ff
                    playerlist += (inactiveSlots[1] + '\n')
                    classlist += (classes[10] + '\n')
                elif message.server.id == '189900848862724096':
                    color[message.channel.id] = 0xcc0000
                    playerlist += (inactiveSlots[2] + '\n')
                    classlist += (classes[10] + '\n')
            # Laplace and Ishana, and possibly any other server that isn't the alliance.
            else:
                splitstr = word.split()
                classRole = splitstr[0]
                if not classRole.startswith('<'):
                    ...
                    if len(splitstr) > 2:
                        for index in range(len(splitstr)):
                            if index == 0 or index == 1:
                                ...
                                player+= splitstr[index] + ' '
                if message.server.id == '159184581830901761':
                    playerlist += (activeSlots[0] + ' ' + player + '\n')
                elif message.server.id == '153346891109761024':
                    playerlist += (activeSlots[1] + ' ' + player + '\n')
                elif message.server.id == '189900848862724096':
                    playerlist += (activeSlots[2] + ' ' + player + '\n')
                classlist += (classRole + '\n')
    ...
    # elif message.server.id == '189900848862724096':
        ...
        try:
            ...
            print(message.author.name + ' Started an MPA on ' + message.server.name)
            MPACount += 1
            await client.send_message(client.get_channel('322466466479734784'), '```css\n' + message.author.name + '#' + message.author.discriminator + ' (ID: ' + message.author.id + ') ' + 'Started an MPA on ' + message.server.name + '\nAmount of Active MPAs: ' + str(MPACount) + '\nTimestamp: ' + str(datetime.now()) + '```')
            print('Amount of Active MPAs: ' + str(MPACount))
            EQPostDict[message.channel.id] = await client.send_message(message.channel, '', embed=em)
