...
# TODO: Change all the adding/changing classes to the new reading from input file method.
# Possibly load classes on startup, then add a command to reload all the classes in the event of a class change now, considering it could be too much of a pain to do.
# Rewrite toonk to read servers from a file to a list, then run commands based on all the servers in a list, and then add command to allow adding servers from a command.
# Clean some code on tonk, remove joke commands to save space. 
...
print ('Loading servers with classes list...\n')
with open("serversWithClasses.txt") as g:
    serversWithClasse = g.readlines()
    serversWithClasses = []
    for i in serversWithClasse:
        serversWithClasses.append(i.strip())
print ('Server classes loaded.\n')    

with open("grayServerSlots.txt") as f:
	inactiveSlotsTemp = f.readlines()
	inactiveSlots = []
		for i in inactiveSlotsTemp:
			inactiveSlots.append(i.strip())
			
with open("activeServerSlots.txt") as f:
	activeSlotsTemp = f.readlines()
	activeSlots = []
		for i in activeSlotsTemp:
			activeSlots.append(i.strip())

print ('Loading classes list...\n')  
  
with open("ishanaClass.txt") as e:
    ishanaclassesread = e.readlines()
    ishanaclasses = []
    for i in ishanaclassesread:
        ishanaclasses.append(i.strip())
with open("laplaceClass.txt") as d:
    laplaceclassesread = d.readlines()
    laplaceclasses = []
    for i in laplaceclassesread:
        laplaceclasses.append(i.strip())
with open("allianceClass.txt") as h:
    allianceclassesread = h.readlines()
    allianceclasses = []
    for i in allianceclassesread:
        allianceclasses.append(i.strip())
...
async def generateList(message, inputstring):
    global MPACount
	global inactiveSlots
	global activeSlots
	global ishanaclasses
	global laplaceclasses
	global serversWithClasses
	global allianceclasses
    global grayServerSlots
    global activeServerSlots
    pCount = 1
    nCount = 1
    sCount = 1
    mpaCount = 1
 ...
 # This is the Ishana format
                    ...
                    if len(splitstr) > 2:
                        for index in range(len(splitstr)):
                            if index == 0 or index == 1:
                                ...
                                player+= ' ' + splitstr[index]
                playerlist += ('<:ishana:337775527836254208>' + ' ' + player + '\n')
                classlist += (classRole + '\n')
    ...
    # Laplace/Any other place that it may take place in 	
        ...
        for word in EQTest[message.channel.id]:
            if (type(word) is PlaceHolder):
                playerlist += ('<:laplace_gray:376517363308298241>' + '\n')
                classlist += ('<:umi1:300340971613454357>' + '\n')
            else:
                splitstr = word.split()
                classRole = splitstr[0]
                if not classRole.startswith('<'):
                    classRole = '<:umi1:300340971613454357>'
                    player = splitstr[0]
                    if len(splitstr) > 2:
                        for index in range(len(splitstr)):
                            if index == 0 or index == 1:
                    ...
                    if len(splitstr) > 2:
                        for index in range(len(splitstr)):
                            if index == 0 or index == 1:
                                ...
                                player+= splitstr[index] + ' '
                playerlist += ('<:laplace:376505794335604747>' + ' ' + player + '\n')
                classlist += (classRole + '\n')
        if len(SubDict[message.channel.id]) > 0:
            playerlist += ('\n**Reserve List**:\n')
            for word in SubDict[message.channel.id]:
        ...
        for word in EQTest[message.channel.id]:
            if (type(word) is PlaceHolder):
                playerlist += ('<:noname:338909184625999872>' + '\n')
                classlist += ('<:bob:331190415430516756>' + '\n')
            else:
                splitstr = word.split()
                # Team name will NEVER be blank. 
                teamName = splitstr[0]
                # if not teamName.startswith('<'):
                    # teamName = '<:noname:338909184625999872>'
                    # player = splitstr[0]
                    # if len(splitstr) > 2:
                        # for index in range(len(splitstr)):
                            # if index == 0 or index == 1:
                            ...
                            # else:
                                # player+= ' ' + splitstr[index]
                #else:
                if roleAdded[message.channel.id] == True:
                    classRole = splitstr[1]
                    player = splitstr[2]
                    classlist += (classRole + '\n')
                    if len(splitstr) > 3:
                        for index in range(len(splitstr)):
                    ...
                    if message.channel.id in EQTest:
                        ...
                        for index, item in enumerate(SubDict[message.channel.id]):
                            if message.author.name in item:
                                ...
                                break
                        if message.server.id in serversWithClasses:
                            serverHasClasses = True
                        if serverHasClasses == True:
                            if message.server.id == '153346891109761024':
                                classes = laplaceclasses
                            elif message.server.id == '159184581830901761':
                                classes = ishanaclasses
                        ...
                        if message.server.id == '189900848862724096':
                            for index in range(len(message.author.roles)):
                                if message.author.roles[index].id == '189914908328984576':
                                    classRole = '<:moonlight:312677123209035793>'
                                    break
                                elif message.author.roles[index].id == '217335164987113472':
                                    classRole = '<:laplace:312671637730361344>'       
                                    break
                                elif message.author.roles[index].id == '310183920069246986':
                                    classRole = '<:pretty:313595086753628160>'
                                    break
                                elif message.author.roles[index].id == '201230397596631040':
                                    classRole = '<:ishana:312673634617655317>'
                                    break
                                elif message.author.roles[index].id == '323370572937560064':
                                    classRole = '<:fuyu:338904004337598475>'
                                    break
                                elif message.author.roles[index].id == '327698239598100480':
                                    classRole = '<:littlewings:338909396446871553>'
                                    break
                                else:
                                    classRole = '<:noname:338909184625999872>'
                        if serverHasClasses == True:
                            if userstr.lower() == 'hu' or userstr.lower() == 'hunter':
                                ...
                                classRole += ' ' + classes[9]
                                roleAdded[message.channel.id] = True
                            elif (userstr.lower() == 'wb' or userstr.lower() == 'weakbullet') and message.server.id == '159184581830901761':
                                classRole += ' ' + classes[10]
                                roleAdded[message.channel.id] = True
                            else:
                                if message.server.id == '159184581830901761':
                                    classRole += ' ' + classes[11]
                                else:
                                    classRole += ' ' + classes[10]
                                roleAdded[message.channel.id] = True
                        else:
                            ...
                            if serverHasClasses == True:
                                if len(splitstr) == 2:
                                    if splitstr[1].lower() == 'hu' or splitstr[1].lower() == 'hunter':
                                        ...
                                        classRole = classes[9]
                                    elif splitstr[1].lower() == 'wb' and message.server.id == '159184581830901761':
                                        classRole = classes[10]
                                else:
                                    if message.server.id == '159184581830901761':
                                        classRole = classes[11]
                ...
                if message.channel.id in EQTest:
                    ...
                    if serverHasClasses == True:
                        if message.server.id == '153346891109761024':
                            ...
                            classes = allianceclasses
                    if serverHasClasses == False:
                        await generateList(message, '```fix\nThis server does not have classes enabled!```')
                        return
                    if message.server.id == '159184581830901761':
                        if userstr == 'hu' or userstr == 'hunter':
                            ...
                            newRole = classes[9]
                            newRoleName = 'Hero'
                        elif userstr == 'wb' and message.server.id == '159184581830901761':
                            newRole = classes[10]
                            newRoleName = 'Weak Bulletor'
                        else:
                            if message.server.id == '159184581830901761':
                                newRole = classes[11]
                            else:
                    ...
                    for index, item in enumerate(EQTest[message.channel.id]):
                        if (type(EQTest[message.channel.id][index]) is PlaceHolder):
                            ...
                            if message.server.id == '189900848862724096':
                                splitstr = EQTest[message.channel.id][index].split()
                                teamName = splitstr[0]
                                newRole2 = splitstr[0] + ' ' + newRole
                                EQTest[message.channel.id].pop(index)
