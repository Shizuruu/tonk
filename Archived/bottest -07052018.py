...
with open(os.path.join('Asset-Tonk',"classes.txt"),'r') as e:
...
with open(os.path.join('Assets-Tonk',"activeServerSlots.txt"),'r') as e:
    assRead = e.readlines()
    activeServerIcons = []
    for i in assRead:
        activeServerIcons.append(i.strip())
        
with open(os.path.join("Assets-Tonk","grayServerSlots.txt"),'r') as e:
    ...
    if message.content.startswith('!'):
        ...
        # Closes all MPAs the bot has ever known to be active at this moment. Easy to use before shutting the bot down.
            ...
            if message.author.id == '153273725666590720':
                ...
                await client.send_message(client.get_channel('322466466479734784'), 'Tonk is {}'.format(restartRole.mention) + '\nRun time: ' + time.strftime('%H hours, %M minutes, %S seconds', time.gmtime(runTime)))
        ...
        #Starts the MPA on the current eq channel. Places the channel name into a dictionary and sets it to be a list. Then fills the list up with 12 placeholder objects.
            ...
            if message.channel.name.startswith('mpa') or message.channel.id == '206673616060940288':
                if message.server.id == '159184581830901761':
                    ...
                    elif userstr == ' oloser' or userstr == ' omegaloser':
                        await client.send_file(message.channel, 'oloser.jpg')
                    elif userstr == ' test':
            ...
            if message.channel.name.startswith('mpa') or message.channel.id == '206673616060940288' or message.author.top_role.permissions.administrator:
                for index in range(len(message.author.roles)):
                    if message.author.roles[index].id == '154465245488742400':
                        ...
                        bypassCheck = True
                        break
                    elif message.author.roles[index].id == '357155081512026125':
                        bypassCheck = True
                        break
        ...
        # Adds a string/name of a player that the Manager wants into the MPA list. Can also take a class role as well.
            ...
            if message.author.top_role.permissions.manage_emojis or message.author.id == '153273725666590720' or message.server.id == '410601412620320819' or message.author.top_role.permissions.administrator:
                userstr = ''
                classRole = ''
                if message.channel.name.startswith('mpa') or message.channel.id == '206673616060940288':
                    if message.channel.id in EQTest:
...
async def on_ready():
    ...
    onlineRole = discord.utils.get(client.get_server('226835458552758275').roles, id='370337403769978880')
    await client.send_message(client.get_channel('322466466479734784'), 'Tonk is now {}'.format(onlineRole.mention) + '\nStartup time: ' + time.strftime('%H hours, %M minutes, %S seconds', time.gmtime(loadupTime)) + '\nConnected to **' + str(connectedServers) + '** servers' + '\nLast Restarted: ' + lastRestart)
@client.event
...
async def on_resumed():
    ...
    await client.send_message(client.get_channel('322466466479734784'), 'Tonk has {}'.format(resumeRole.mention) + '\nConnected to **' + str(connectedServers) + '** servers')
