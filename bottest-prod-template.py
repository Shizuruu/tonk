...
async def generateList(message, inputstring):
    ...
    try:
        ...
        await client.get_channel(OtherIDDict['ControlPanel']).send(
            '```css\n' + message.author.name + '#' + str(message.author.discriminator) + ' (ID: ' + str(
                message.author.id) + ') ' + 'Started an MPA on ' + message.guild.name + '\nAmount of Active MPAs: ' + str(
                ...
                if message.author.top_role.permissions.manage_emojis or message.author.id == OtherIDDict or message.author.top_role.permissions.administrator or message.author == client.user:
                    try:
                        ...
                        if message.channel.id in mpaExpirationConfig[str(message.guild.id)]:
                            expirationDate[message.channel.id] = (
        ...
        if message.channel.id in mpaChannels[str(message.guild.id)]:
            if message.channel.id in EQTest:
                try:
                    ...
                    await client.get_channel(OtherIDDict['ControlPanel']).send(
                        '```diff\n- ' + message.author.name + '#' + message.author.discriminator + ' (ID: ' + str(
                            message.author.id) + ') ' + 'Closed an MPA on ' + message.guild.name + '\n- Amount of Active MPAs: ' + str(
                            MPACount) + '\nTimestamp: ' + str(datetime.now()) + '```')
            ...
            if ctx.channel.id in EQTest:
                ...
                if len(mpaArg) > 2:
                    if len(mpaArg) == 4:
                        ...
                        if classSplit[0] not in heroClasses and classSplit[
                            1] in subbableHeroClasses or mpaClass2 == BlankMpaClass:
                        ...
                        if len(mpaArg) > 2:
                            if len(mpaArg) == 4:
                                ...
                                if classSplit[0] not in heroClasses and classSplit[
                                    1] in subbableHeroClasses or mpaClass2 == BlankMpaClass:
                                    classRole = classes[mpaClass1] + classes[mpaClass2]
                                elif classSplit[0] in heroClasses or classSplit[
                                    1] in heroClasses or mpaClass2 == BlankMpaClass:
                                    classRole = classes[mpaClass1]
                                else:
                                    classRole = classes[mpaClass1] + classes[mpaClass2]
        ...
        if ctx.channel.id in EQTest:
            ...
            if len(mpaArg) > 2:
                if len(mpaArg) == 4 and mpaArg != 'none':
                    ...
                    if mpaClass1 == mpaClass2:
                        mpaClass2 = classMatch.findClass('NoClass')
                    elif classSplit[0] not in heroClasses and classSplit[1] in subbableHeroClasses or str(
                            mpaClass2) == str(BlankMpaClass):
                        classRole = classes[mpaClass1] + classes[mpaClass2]
                    elif classSplit[0] in heroClasses or classSplit[1] in heroClasses or str(mpaClass2) == str(
                            BlankMpaClass):
